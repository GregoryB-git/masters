  ...
  implements Producer<EncodedImage>
{
  ...
  public static float determineResizeRatio(ResizeOptions paramResizeOptions, int paramInt1, int paramInt2)
  {
    ...
    float f3 = height;
    float f4 = paramInt2;
    f3 = Math.max(f1, f3 / f4);
    float f5 = maxBitmapSize;
    f1 = f3;
    if (f2 * f3 > f5) {
      f1 = f5 / f2;
    }
  ...
  private static int getForceRotatedInvertedExifOrientation(RotationOptions paramRotationOptions, EncodedImage paramEncodedImage)
  {
    ...
    paramEncodedImage = INVERTED_EXIF_ORIENTATIONS;
    int j = paramEncodedImage.indexOf(Integer.valueOf(i));
    if (j >= 0)
    {
      i = 0;
      ...
      return ((Integer)paramEncodedImage.get((i / 90 + j) % paramEncodedImage.size())).intValue();
    }
  ...
  private static int getRotationAngle(RotationOptions paramRotationOptions, EncodedImage paramEncodedImage)
  {
    ...
    return (paramRotationOptions.getForcedAngle() + i) % 360;
  }
  ...
  private static int getSoftwareNumerator(ImageRequest paramImageRequest, EncodedImage paramEncodedImage, boolean paramBoolean)
  {
    ...
    int k;
    if (paramBoolean) {
      k = getForceRotatedInvertedExifOrientation(paramImageRequest.getRotationOptions(), paramEncodedImage);
    } else {
      k = 0;
    }
    int m = 1;
    if ((i == 90) || (i == 270) || (k == 5) || (k == 7)) {
      j = 1;
    }
    if (j != 0) {
    ...
    j = roundNumerator(determineResizeRatio(localResizeOptions, k, j), roundUpFraction);
    if (j > 8) {
      return 8;
    }
    if (j < 1) {
  ...
  {
    return (int)(paramFloat1 * 8.0F + paramFloat2);
  }
  ...
  {
    boolean bool;
    if ((!paramRotationOptions.canDeferUntilRendered()) && ((getRotationAngle(paramRotationOptions, paramEncodedImage) != 0) || (shouldRotateUsingExifOrientation(paramRotationOptions, paramEncodedImage)))) {
      bool = true;
    } else {
      bool = false;
    }
  ...
  {
    mInputProducer.produceResults(new TransformingConsumer(paramConsumer, paramProducerContext), paramProducerContext);
  }
    ...
    extends DelegatingConsumer<EncodedImage, EncodedImage>
  {
    ...
    public TransformingConsumer(final ProducerContext paramProducerContext)
    {
      ...
      mJobScheduler = new JobScheduler(ResizeAndRotateProducer.access$100(ResizeAndRotateProducer.this), local1, 100);
    ...
    public void onNewResultImpl(@Nullable EncodedImage paramEncodedImage, int paramInt)
    {
      ...
      Object localObject = ResizeAndRotateProducer.access$600(mProducerContext.getImageRequest(), paramEncodedImage, ResizeAndRotateProducer.access$500(ResizeAndRotateProducer.this));
