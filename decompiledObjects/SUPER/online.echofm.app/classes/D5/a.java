package D5;

import E5.a.d;
import E5.a.e;
import E5.p;
import io.flutter.embedding.engine.FlutterJNI;
import io.flutter.embedding.engine.FlutterJNI.a;
import io.flutter.view.i.g;
import java.util.HashMap;
import t5.b;

public class a
{
  public final E5.a a;
  public final FlutterJNI b;
  public b c;
  public final a.d d;
  
  public a(w5.a parama, FlutterJNI paramFlutterJNI)
  {
    a locala = new a();
    d = locala;
    parama = new E5.a(parama, "flutter/accessibility", p.a);
    a = parama;
    parama.e(locala);
    b = paramFlutterJNI;
  }
  
  public void b(int paramInt, i.g paramg)
  {
    b.dispatchSemanticsAction(paramInt, paramg);
  }
  
  public void c(int paramInt, i.g paramg, Object paramObject)
  {
    b.dispatchSemanticsAction(paramInt, paramg, paramObject);
  }
  
  public void d()
  {
    b.setSemanticsEnabled(false);
  }
  
  public void e()
  {
    b.setSemanticsEnabled(true);
  }
  
  public void f(int paramInt)
  {
    b.setAccessibilityFeatures(paramInt);
  }
  
  public void g(b paramb)
  {
    c = paramb;
    b.setAccessibilityDelegate(paramb);
  }
  
  public class a
    implements a.d
  {
    public a() {}
    
    public void a(Object paramObject, a.e parame)
    {
      if (a.a(a.this) == null)
      {
        parame.a(null);
        return;
      }
      HashMap localHashMap1 = (HashMap)paramObject;
      paramObject = (String)localHashMap1.get("type");
      HashMap localHashMap2 = (HashMap)localHashMap1.get("data");
      StringBuilder localStringBuilder = new StringBuilder();
      localStringBuilder.append("Received ");
      localStringBuilder.append((String)paramObject);
      localStringBuilder.append(" message.");
      b.f("AccessibilityChannel", localStringBuilder.toString());
      ((String)paramObject).hashCode();
      int i = ((String)paramObject).hashCode();
      int j = -1;
      switch (i)
      {
      default: 
        break;
      case 114203431: 
        if (((String)paramObject).equals("longPress")) {
          j = 4;
        }
        break;
      case 97604824: 
        if (((String)paramObject).equals("focus")) {
          j = 3;
        }
        break;
      case 114595: 
        if (((String)paramObject).equals("tap")) {
          j = 2;
        }
        break;
      case -649620375: 
        if (((String)paramObject).equals("announce")) {
          j = 1;
        }
        break;
      case -1140076541: 
        if (((String)paramObject).equals("tooltip")) {
          j = 0;
        }
        break;
      }
      switch (j)
      {
      default: 
        break;
      case 4: 
        paramObject = (Integer)localHashMap1.get("nodeId");
        if (paramObject != null) {
          a.a(a.this).f(((Integer)paramObject).intValue());
        }
        break;
      case 3: 
        paramObject = (Integer)localHashMap1.get("nodeId");
        if (paramObject != null) {
          a.a(a.this).e(((Integer)paramObject).intValue());
        }
        break;
      case 2: 
        paramObject = (Integer)localHashMap1.get("nodeId");
        if (paramObject != null) {
          a.a(a.this).g(((Integer)paramObject).intValue());
        }
        break;
      case 1: 
        paramObject = (String)localHashMap2.get("message");
        if (paramObject != null) {
          a.a(a.this).a((String)paramObject);
        }
        break;
      case 0: 
        paramObject = (String)localHashMap2.get("message");
        if (paramObject != null) {
          a.a(a.this).d((String)paramObject);
        }
        break;
      }
      parame.a(null);
    }
  }
  
  public static abstract interface b
    extends FlutterJNI.a
  {
    public abstract void a(String paramString);
    
    public abstract void d(String paramString);
    
    public abstract void e(int paramInt);
    
    public abstract void f(int paramInt);
    
    public abstract void g(int paramInt);
  }
}

/* Location:
 * Qualified Name:     D5.a
 * Java Class Version: 6 (50.0)
 * JD-Core Version:    0.7.1
 */